...
function renderMonthFilters() {
  const keys = Array.from(
    new Set(
      fixtures
        .filter((fixture) => !fixture.pending)
...
function render() {
  const filtered = fixtures
    ...
    .filter((fixture) => activeMonth === "all" || monthKey(fixture) === activeMonth)
...
function createCard(fixture) {
  ...
  node.querySelector(".scoreline").textContent = scorelineText(fixture);
...
function scorelineText(fixture) {
  if (fixture.pending) return "Awaiting fixture release";
  if (fixture.status === "C" && fixture.score) {
    return `${fixture.home} ${fixture.score.home} - ${fixture.score.away} ${fixture.away}`;
  }
  ...
  positionChart.innerHTML = `
    <svg viewBox="0 0 ${width} ${height}" role="img" aria-label="Manchester United weekly league position">
      ...
      ${history
        .map(
          (point) =>
            `<circle cx="${x(point.gameweek)}" cy="${y(point.position)}" r="5" fill="#c70101"><title>GW ${point.gameweek}: ${point.position}</title></circle>`,
        )
        .join("")}
    </svg>
  `;
...
async function refreshFixtures() {
  ...
  try {
    const [fetchedGroups, fetchedStandings, fetchedTopScorer] = await Promise.all([
